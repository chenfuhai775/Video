...
function Translator()
{
	...
	*************************************************/
	this.initLanguageSelect = function(szLanguage) {
		...
		setTimeout(function() { $('#LanguageSelect').val(that.szCurLanguage); }, 10); // IE6需延迟设置
	...
	*************************************************/
	this.getLanguageXmlDoc = function(szXmls, szLanguage, bMergeCommon) {
		...
		var lxd = $.ajax({
			url: "../assets/xml/" + this.szCurLanguage + "/" + szXmls[0] + ".xml?version=" + ((typeof global_config !== "undefined")?global_config.web_version:window.parent.global_config.web_version),
			type: "get",
			async: false,
			cache: true,
			dataType: "xml",
	...
	*************************************************/
	this.translatePage = function(languageXmlDoc, doc, szKey) {
		...
		this.translateElements(languageXmlDoc, doc, "span", "innerHTML", szKey);
		var that = this;
