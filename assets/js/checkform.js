...
function CheckDeviceName(strInfo, tipsId, iNull) {
	var szAreaNameInfo = "<span class='am-icon-times'></span>&nbsp;";
	if(iNull == 0) {
		if($.isEmpty(strInfo)) {
			szAreaNameInfo += g_oCommon.getNodeValue("DevNameNullTips");
			$("#" + tipsId).html(szAreaNameInfo);    
			return false;
		}
	}
	var forbidChar = ["'",":","*","?","<",">","|", "/", "%","\\",'"', "\""]; //包含特殊字符时提示
	for(var i = 0;i < forbidChar.length ; i++) { 
  		if(strInfo.indexOf(forbidChar[i]) >= 0) { 
			szAreaNameInfo += g_oCommon.getNodeValue("DevNameWrongCharTips") + " / \\ : * ? ' \" < > | % ";
			$("#" + tipsId).html(szAreaNameInfo);    
      		return false;
	  	} 
	} 
	if($.lengthw(strInfo) > 32) {
		szAreaNameInfo += g_oCommon.getNodeValue("DevNameLengthTips");
		$("#" + tipsId).html(szAreaNameInfo);    
		return false;
	}
	$("#" + tipsId).html("");    
...
function CheackServerIDIntNum(strInfo, tipsId, szName, iMin, iMax) {
	...
	if(!$.isCosinaIntNum(strInfo,iMin,iMax)) {
		szTipsInfo += g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("RangeTips") + iMin + "-" + iMax;
		$("#" + tipsId).html(szTipsInfo); 
		return false;
	}
	$("#" + tipsId).html(""); 
...
function CheackStringLenth(strInfo, tipsId, szName, iLen) {
	...
	if(strInfo !== "") {
		if($.lengthw(strInfo) > iLen) {
			szTipsInfo += g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("LengthTips") + iLen;
			$("#" + tipsId).html(szTipsInfo); 
			return false;
		}
	}	
...
function CheckDIPadd(strInfo, tipsId, szName) {
	...
	if(!$.isIpAddress(strInfo)) {
		szTipsInfo += g_oCommon.getNodeValue("WrongTips") + g_oCommon.getNodeValue(szName);
		$("#" + tipsId).html(szTipsInfo); 
		return false;
	}
	if(!$.isDIpAddress(strInfo)) {
		szTipsInfo += g_oCommon.getNodeValue("DIPAddInvalidTips");
		$("#" + tipsId).html(szTipsInfo); 
		return false;
	}
	$("#" + tipsId).html(""); 
...
function CheckMacadd(iSetId, iSetValue, tipsId) {
	chkstr = iSetValue;
   	if(chkstr.length === 0) {
		$("#" + iSetId).val("00:00:00:00:00:00");     
      	$("#" + tipsId).html(""); 
		return true;
    }
	var szTipsInfo = "<img src='../images/tips.png' class='picAndtext'>&nbsp;";
    var pattern="/^([0-9A-Fa-f]{2})(-[0-9A-Fa-f]{2}){5}|([0-9A-Fa-f]{2})(:[0-9A-Fa-f]{2}){5}/";
    eval("var pattern=" + pattern);
    var add_p1 = pattern.test(chkstr);
    if(!add_p1) {
		szTipsInfo += g_oCommon.getNodeValue("MacAddInvalidTips");
		$("#" + tipsId).html(szTipsInfo); 
		return false;
    }
	$("#" + tipsId).html(""); 
...
function CheckDevUserName(strInfo, tipsId, szName, iNull) {
	...
	if(iNull === 0) {
		if($.isEmpty(strInfo)) {
			szAreaNameInfo += g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("NullTips");
			$("#" + tipsId).html(szAreaNameInfo);    
			return false;
		}
	}
	...
	for(var i = 0;i < forbidChar.length ; i++) { 
  		if(strInfo.indexOf(forbidChar[i]) >= 0) {
			szAreaNameInfo += g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("WrongCharTips") + " / \\ : * ? ' \" < > | % ";
			$("#" + tipsId).html(szAreaNameInfo);    
      		return false;
	  	}
	} 
	if($.lengthw(strInfo) > 16) {
		szAreaNameInfo += g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("UserNameLengthTips");
		$("#" + tipsId).html(szAreaNameInfo);    
		return false;
	}
	$("#" + tipsId).html("");    
...
function CheckCharName(strInfo, tipsId) {
	...
	if($.lengthw(strInfo) > 44) {
		szAreaNameInfo += g_oCommon.getNodeValue("CharNameLengthTips");
		$("#" + tipsId).html(szAreaNameInfo);    
		return false;
	}
	$("#" + tipsId).html("");    
...
function CheckMaskIP(strInfo, tipsId, szName) {
	...
	if(!$.isIpAddress(strInfo)) {
		szAreaNameInfo += g_oCommon.getNodeValue("WrongTips") + g_oCommon.getNodeValue(szName);
		$("#" + tipsId).html(szAreaNameInfo); 
		return false;
	}
	...
	if(-1 != ip_binary.indexOf("01")) {
		szAreaNameInfo += g_oCommon.getNodeValue("MaskAddInvalidTips");
		$("#" + tipsId).html(szAreaNameInfo);    
		return false;
	}
	$("#" + tipsId).html(""); 
...
/*************************************************
  ...
  Return:		bool:true false
*************************************************/
function checkNameAndLength(strInfo, tipsId, szName, iNull, iLength) {
	var szAreaNameInfo = "<span class='am-icon-times'></span>&nbsp;";
	if(iNull === 0) {
		if($.isEmpty(strInfo)) {
			szAreaNameInfo += g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("NullTips");
	...
	if($.lengthw(strInfo) > iLength) {
		szAreaNameInfo += g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("LengthTips") + iLength;
		$("#" + tipsId).html(szAreaNameInfo);    
		return false;
	}
	$("#" + tipsId).html("");    
...
function CheackStringLenthNull(strInfo, tipsId, szName, iLen) {
	var szTipsInfo = "<span class='am-icon-times'></span>";
	if($.isEmpty(strInfo)) {
		szTipsInfo += g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("NullTips");
		$("#" + tipsId).html(szTipsInfo);    
		return false;
	}
	if(strInfo !== "") {
...
function CheckFilePathStrLen(szFilePath, tipsID, szName, iDiskNo, iLen) {
	var szTipsInfo = "<span class='am-icon-times'></span>&nbsp;";
	if(szFilePath === "") {
		szTipsInfo += g_oCommon.getNodeValue("laDiskNumer") + iDiskNo + g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("NullTips");
		$("#" + tipsID).html(szTipsInfo); 
		return false;
	}
	if($.lengthw(szFilePath) > iLen) {
		szTipsInfo += g_oCommon.getNodeValue("laDiskNumer") + iDiskNo + g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("LengthTips") + iLen;
		$("#" + tipsID).html(szTipsInfo); 
		return false;
	}
	$("#" + tipsID).html(""); 
...
/*************************************************
  ...
  Return:		bool:true false
*************************************************/
function CheckNamePlus(strInfo, tipsId, szName, iNull, iLength) {
	var szAreaNameInfo = "<span class='am-icon-times'></span>&nbsp;";
	if(iNull === 0) {
		if($.isEmpty(strInfo)) {
			szAreaNameInfo += g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("NullTips");
	...
	if (!strInfo.match(usern) && !$.isEmpty(strInfo)) {
		szAreaNameInfo += g_oCommon.getNodeValue("UserNameCharTips");
		$("#" + tipsId).html(szAreaNameInfo); 
		return false;
    }
...
function CheckIPAddress(strInfo, tipsId, szName) {
	...
	if(!$.isIPv6(strInfo) &&　(!$.isIpAddress(strInfo) || !$.isDIpAddress(strInfo))) {
		szTipsInfo += g_oCommon.getNodeValue("WrongTips") + g_oCommon.getNodeValue(szName);
		$("#" + tipsId).html(szTipsInfo); 
		return false;
	}
...
/*************************************************
  Function:    	CheckPortValueValidEx
  Description:	检查端口号是否在合法范围内
  Input:        strInfo:传入的参数
  				tipsId:提示信息
				szName:标题
				iValue: 可设置的特殊端口号(值不在iMin~iMax范围内)
				iMin:最小数
				iMax:最大数
  Output:      	无
  Return:		bool:true false
*************************************************/
function CheckPortValueValidEx(strInfo, tipsId, szName, iValue, iMin, iMax) {
	var szTipsInfo = "<span class='am-icon-times'></span>&nbsp;";
	if(strInfo === "") {
		szTipsInfo += g_oCommon.getNodeValue("InputTips") + g_oCommon.getNodeValue(szName); 
		$("#" + tipsId).html(szTipsInfo); 
	...
	if(!$.isCosinaIntNum(strInfo, iMin, iMax) && (strInfo != iValue)) {
		szTipsInfo += g_oCommon.getNodeValue(szName) + g_oCommon.getNodeValue("RangeTips") + iValue + "," + iMin + "-" + iMax;
		$("#" + tipsId).html(szTipsInfo); 
		return false;
	}
	$("#" + tipsId).html(""); 
...
function CheckMulticastIP(strInfo,tipsId,szName)
{
	...
	{
		szTipsInfo += g_oCommon.getNodeValue("jsMulticastRange");
		$("#" + tipsId).html(szTipsInfo); 
		return false;
	}
	$("#" + tipsId).html(""); 
